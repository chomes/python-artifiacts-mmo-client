...
class Character:
    def __init__(self, data: dict[str, str | int]):
        ...
        self.cooldown_expiration = datetime.fromisoformat(data['cooldown_expiration'].replace('Z', '+00:00'))
        ...
        self.inventory_slots = [
            {'slot': i + 1, 'code': data.get(f'inventory_slot{i + 1}', ''), 'quantity': data.get(f'inventory_slot{i + 1}_quantity', 0)}
            for i in range(20)
        ]
