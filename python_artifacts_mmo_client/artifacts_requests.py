...
class ArtifactsRequests:
    ...
    def get(self, endpoint: str) -> dict[str, str]:
        response: requests.Response = self.requests_module.get(f"{API_ENDPOINT}{endpoint}", headers=HEADERS)
        logger.debug(f"Response from GET request {response.json()}")
        return response.json()

    def post(self, endpoint: str, data: dict[str, str]) -> dict[str, str]:
        response: requests.Response = self.requests_module.post(f"{API_ENDPOINT}{endpoint}", headers=HEADERS, json=data)
